...
if(isset($_GET['link']) and isset($_GET['id']))
{
    ...
    if (isset($_POST['reg'])){
        ...
        $sql = "UPDATE `clearance_table` SET `first_name` = '$firstname', `middle_name` = '$middlename', `last_name` = '$lastname', `age` = '$age', `gender` = '$gender', `address` = '$address', `birthday` = '$birthday', `birthplace` = '$birthplace', `status` = '$status', `spouse` = '$spouse', `spouse_age` = '$spouse_age', `year_stay` = '$year_stay', `voter` = '$voter', `voter_number` = '$voterNo', `house` = '$house', `occupation` = '$occupation', `companyname` = '$companyname', `contact_number` = '$contactnumber' WHERE `clearance_table`.`id` = $id";
        mysqli_query($con,$sql);
        header('location:admin.php?link=clearance&update=clearance');
